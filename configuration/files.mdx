...
<Info>
  The deployment POST request (after archive compression) has a size limit of 50
  MB.
</Info>
...
<Note>
  Specifying only a folder name, say `"static"`, does not work. Use `"static/*"`
  instead to include its contents.
</Note>
